...
constructor(private var application: Application, private val historyRepository: HistoryRepository) {
    ...
    fun suggestionsTransformer(): Transformer<String, List<SuggestionItem>> {
        return Transformer {
            ...
            return@Transformer it
                    ...
                    .switchMap {
                        val googleSuggestions = Observable.just(it).compose(googleTransformer())
                        val historySuggestions = Observable.just(it).compose(historyTransformer())

                        return@switchMap Observable.zip(googleSuggestions, historySuggestions, deviceSuggestions,
    ...
    private fun googleTransformer(): Transformer<String, List<SuggestionItem>> {
        return Transformer {
            ...
            } else return@Transformer it
                    .compose(RxSuggestions.suggestionsTransformer(suggestionsLimit))
                    .map { it.map { GoogleSuggestionItem(it) } as List<SuggestionItem> }
    ...
    private fun historyTransformer(): Transformer<String, List<SuggestionItem>> {
        return Transformer {
            return@Transformer it
                    ...
                    .map { it.map { HistorySuggestionItem(it) }.take(suggestionsLimit) as List<SuggestionItem> }
                    .onErrorReturn { emptyList() }
        }
    }
